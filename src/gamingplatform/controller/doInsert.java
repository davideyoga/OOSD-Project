...
public class doInsert extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!"XMLHttpRequest".equals(request.getHeader("X-Requested-With"))) {
            Logger.getAnonymousLogger().log(Level.WARNING, "[Add user] non è una chiamata ajax");
        ...
        try {
            ...
            switch (item) {
                ...
                case "user":
                    ...
                    if (isNull(username) || isNull(name) || isNull(surname) || isNull(email) || isNull(password) ||
                        ...
                        Logger.getAnonymousLogger().log(Level.WARNING, "[Add user] Parametri POST non validi ");
                        //torno KO alla chiamata servlet
                        response.getWriter().write("KO");
                        return;
                    }
                    ...
                    if (isNull(avatarName)) {
                        ...
                        Logger.getAnonymousLogger().log(Level.WARNING, "[Add user] Upload file fallito");
                        //torno KO alla chiamata servlet
                        response.getWriter().write("KO");
                        return;
                    }
                ...
                default:
                    ...
                    Logger.getAnonymousLogger().log(Level.WARNING, "[doAdd: item = " + item + "] caso default nello switch");
                    //torno KO alla chiamata servlet
                    response.getWriter().write("KO");
                    return;
            }
            ...
            Logger.getAnonymousLogger().log(Level.WARNING, "[doAdd: item = " + item + "] " + e.getMessage());
            //torno KO alla chiamata servlet
            response.getWriter().write("KO");
            return;
        }
